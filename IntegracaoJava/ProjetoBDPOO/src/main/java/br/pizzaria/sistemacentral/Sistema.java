...
public class Sistema {
    ...
    private void finalizarPedido(Pedido pedido) {
        // 1. Insere o pedido e pega o ID gerado
        int idPedido = pedidoDAO.inserirPedido(pedido, pedido.getId_cliente());

        if (idPedido != -1) {
            // 2. Agora insere cada pizza na tabela item_pedido
            for (Pizza pizza : pedido.getPizzas()) {
                pedidoDAO.inserirItemPedido(idPedido, pizza.getIdPizza(), 1); // 1 = quantidade
            }
